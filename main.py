...
class add_address_modal(Modal, title = '新增外送地址'):
    ...
    async def on_submit(self, interact: dc.Interaction):
        ...
        address = self.delivery_address.value
...
class search_modal(Modal, title = '查詢餐廳'):
  _input = dc.ui.TextInput(label = '請輸入你要外送的餐廳', required = True)
  async def on_submit(self, interact: dc.Interaction):
      global address
      position = json.loads(rq.get('https://api.tomtom.com/search/2/geocode/' + address + '.json?key=' + os.getenv('TOMTOMAPIKEY') + '&countrySet=TW' + '&language=zh-TW').text)
...
async def search(interact: dc.Interaction):
    ...
    if address == '':
      ...
      await interact.response.send_modal(search_modal())
